...
def main(cookie_file, url, low_quality, mp3, store_urls, store_urls_file, no_download, is_course_range, range_start,
         range_end):
    config_logging()
    # args = parse_args()
    ...
    course_page = requests.get(url, cookies=cookies)
    ...
    if store_urls:
        urls_file = open(store_urls_file, 'w')
    ...
    for i in download_range:
        ...
        final_extension = "mp3" if mp3 else "mp4"
        ...
        if len(all_download_links) > 0:
            ...
            if len(all_download_links) > 1:
                ...
                download_link = LQ_download_link if (low_quality or mp3) else HQ_download_link
            else:
